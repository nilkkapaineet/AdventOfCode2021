...
for (let i=0; i<binary[0].length-1; i++) {
  let mcb = mostCommonBit(tempBin, i)
  tempBin = findStringsWithGivenBitInPosition(tempBin, i, mcb)  
  if (tempBin.length == 1) {
    break
  }
}
...
let lcv = binary 
for (let i=0; i<binary[0].length-1; i++) {
  let lcb = leastCommonBit(lcv, i)
  lcv = findStringsWithGivenBitInPosition(lcv, i, lcb)  
  if (lcv.length == 1) {
    break
  }
}
...
function findStringsWithGivenBitInPosition(strs, position, mostCommonBit) {
  let returnArray = []
  strs.forEach(element => {
    if (parseInt(element[position]) == mostCommonBit) {
